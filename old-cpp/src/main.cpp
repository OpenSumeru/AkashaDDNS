...
int main(int argc, char **argv)
{
    ...
    std::function<std::string(httplib::Client & client, std::string zoneId, std::string dnsRecordId,
                              std::string ipAddress, std::string name)>
    ...
    std::string ipAddress =
        getResult(client, "/zones/" + zoneId + "/dns_records/" + dnsRecordId)["content"].get<std::string>();
        ...
        if (rbx != ipAddress)
        {
            ...
            if (reciveIp == rbx)
            {
                ...
                try
                {
                    ...
                    {
                        std::cout << "\033[0;32mLocal IP: \033[0m" << ipAddress << "\033[0;32m From: \033[0m"
                                  << rax["country_code"].get<std::string>() << "\033[0;32m Belong: \033[0m"
                                  << rax["organization"].get<std::string>() << "\033[0;32m Longitude: \033[0m"
                                  << rax["longitude"].get<std::string>() << "\033[0;32m Latitude: \033[0m"
                                  << rax["latitude"].get<std::string>() << std::endl;
                    }
                }
                catch (std::exception &error)
                {
