...
CloudflareProvider::CloudflareProvider(std::shared_ptr<IExternalService> externalService,
                                       std::shared_ptr<ILogger> logger)
    : m_externalService(externalService), m_logger(logger)
{
}

void CloudflareProvider::updateDNS(const std::string &recordName, const std::string &domain, const std::string &ip)
{
...
std::string CloudflareProvider::getCurrentDNSIP(const std::string &recordName, const std::string &domain)
{
    std::string zoneID = m_externalService->getZoneID(domain);
    if (zoneID.empty())
    {
